...
| **Pixel shift (OLED protection)** | Slowly drifts the screensaver content a few pixels once a minute to spread wear across OLED pixels. Applies to the clock, media, and website types; the black overlay needs no protection (its pixels are off). Default off |
| **Media source** *(Media type only)* | Use the **Browse** button to pick a file, folder, or camera from the HA media browser; the selected `media-source://` URI is shown in the read-only field next to it. Folders cycle through their playable contents; cameras stream live over WebRTC with sub-second latency (falling back to MJPEG when the camera has no WebRTC provider); images cross-fade on transitions. Photos are fitted to the screen automatically: one whose aspect ratio is within a sliver of the screen's is zoomed to fill it edge to edge, and anything further off (a portrait shot, a 4:3 photo on a 16:9 tablet) is shown whole over a blurred, dimmed copy of itself instead of black bars. Camera feeds are zoomed the same way but never get the blurred backdrop |
...
| **Include subfolders** *(Media type, folders only)* | Also play media from subfolders of the selected folder - combine with shuffle for random images from a whole photo library. Very large trees are capped (12000 items, 200 folders, 5 levels deep) so activation stays fast; a debug log notes when the cap is hit. Default off |
...
| **Clock size** *(Digital clock type only)* | Scales the time and date from 50% to 300% of the default size. The default size is proportional to the browser viewport, which browsers and kiosk apps report differently, so the same clock can look smaller on some tablets (notably iPads) - raise it there to match your other devices. Stored per browser like the rest of the panel settings |
...
| **External screensaver** *(screensaver disabled only)* | A `switch` or `input_boolean` that's forced off for the duration of each voice interaction, then left alone so its owner (typically Fully Kiosk) can resume its own idle timer. Useful to keep Fully Kiosk's screensaver from covering the voice UI mid-conversation. Ignored inside Kiosk Satellite, which pauses its own screensaver for the whole voice interaction (its Screensaver switch is a master enable, so forcing it off would leave the screensaver disabled) |
| **Kiosk Browser Integration -> Screen brightness while active** | Hardware backlight level (0-100%) while the screensaver is showing. Works in Kiosk Satellite, Fully Kiosk (Android), and Kiosker Pro (iOS) with the JavaScript integration enabled. The previous brightness is restored on dismiss. 0% = fully dark, 100% = leave the backlight untouched (default) |
| **Kiosk Browser Integration -> Dismiss on motion** | Treat the kiosk app's camera-based motion detection as activity: motion dismisses the screensaver and also resets the idle timer, so the screensaver won't activate while someone is moving in front of the camera. Kiosk Satellite and Fully Kiosk (Kiosker Pro has no motion API). Requires motion detection to be enabled in the kiosk app's settings. Default off |
